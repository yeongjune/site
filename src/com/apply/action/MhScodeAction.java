 ...
 */
@Controller
@RequestMapping("mh_scode")
public class MhScodeAction {
	@Autowired
	...
	public String toEdit(ModelMap modelMap,Integer id, Integer examType){
		modelMap.put("id", id);
		modelMap.put("examType", examType);
		return "apply/scode/mhEdit";
	}
	...
	public String selectExam(HttpServletRequest request,ModelMap modelMap, Integer examType){
	...
	public void save(HttpServletRequest request,HttpServletResponse response,
			String id,Double interviewScore,Integer admit,Integer  interview, Integer examType){
		...
		try {
			...
			int com = 0;
			if(examType > 0){
				com= this.zdStudentService.update(studentMap);
			}else{
				com= this.zdStudent2Service.update(studentMap);
			}
			if (com>0) {
	...
	public void saveAdmit(HttpServletRequest request,HttpServletResponse response,
			String ids,Integer admit,String checkRemark, Integer examType){
		Map<String, Object> result=new HashMap<String, Object>();
		result.put("code", Init.FAIL);
		result.put("msg", "操作失败");
		try {
			int com;
			if(examType > 0){
				com= this.zdStudentService.updateStudentAdmit(ids,checkRemark, admit);
			}else{
				com= this.zdStudent2Service.updateStudentAdmit(ids,checkRemark, admit);
			}
			if (com>0) {
				result.put("code", Init.SUCCEED);
				result.put("msg", "操作成功");
